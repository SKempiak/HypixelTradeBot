...
public class TradeCore {
	...
	String productGET = "https://api.hypixel.net/skyblock/bazaar/product?key=[apiKey]&product=[productId]";
	String productLIST = "https://api.hypixel.net/skyblock/bazaar/products?key=[apiKey]";
	String string = "api.hypixel.net/skyblock/bazaar/product?key=YOUR_API_KEY&productId=PRODUCT_ID";
	...
	private void getProductINFO(String prodId) {
		try {
			String get = "https://api.hypixel.net/skyblock/bazaar/products?key=" + APIKey + "&productId=" + prodId;
			BufferedReader br = makeGetRequest(get);
			String response = parseResponse(br);
			System.out.println(response);
		} catch (Exception e) {
